...
import { Upload as UploadIcon, FileWarning } from 'lucide-react-native';
...
// Define component directly and export as default immediately
export default function UploadScreen() {
  ...
  // Create a touchable element with proper type assertions
  ...
  const handleFilePick = async () => {
    try {
      ...
      if (file.size === undefined || file.size > MAX_FILE_SIZE) {
        setError('File size is invalid or exceeds 50MB');
        return;
      }
  ...
  React.useEffect(() => {
    if (listItems.length > 0 && totalCost > 0) {
      const updatedItems = listItems.map((item: ListItemType) => 
  ...
  React.useEffect(() => {
    if (error) {
      ...
      const hasErrorItem = listItems.some((item: ListItemType) => item.type === 'error');
      
      if (!hasErrorItem) {
        setListItems([...listItems, { type: 'error' as const, message: error }]);
      } else {
        // Update existing error
        const updatedItems = listItems.map((item: ListItemType) => 
          item.type === 'error' ? { type: 'error' as const, message: error } : item
        );
        setListItems(updatedItems);
      }
    } else {
      // Remove error items if error is cleared
      setListItems(listItems.filter((item: ListItemType) => item.type !== 'error'));
    }
  }, [error]);
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      {createTouchable({
        ...
        children: (
          <>
            <UploadIcon size={24} color={theme.text} />
      ...
      <FlatList
        ...
        keyExtractor={(item: ListItemType, index: number) => `${item.type}-${index}`}
...
// Add explicit display name
UploadScreen.displayName = 'UploadScreen';
